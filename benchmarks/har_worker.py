"""One isolated reducer on common HAR train/test features; no labels are loaded.

Invoked by benchmark_har.py. Upstream TopoAE uses unchanged official model/loss
...
def main():
    ...
    with threadpool_limits(limits=1):
        if args.method == 'pca':
            ...
            model = DeepTDA(standardize=False, missing_indicators=False, seed=args.seed,
                steps=args.steps, warmup_steps=min(60, args.steps//10), batch_size=256,
                h0_size=128, h1_size=64, subset_bank_size=64, landmark_size=512,
                evaluation_size=32, validation_interval=max(1, args.steps+1),
                lambda_h1=0. if args.method=='no_h1' else .1)
